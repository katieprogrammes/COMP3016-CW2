...
public:
    ...
    float GetWorldRadius(const glm::vec3& scale) const
    {
        return boundingRadius * glm::max(glm::max(scale.x, scale.y), scale.z);
    }

    glm::vec3 GetWorldCenter(const glm::vec3& position,
        const glm::vec3& scale) const
    {
        glm::vec3 localCenter = (minBounds + maxBounds) * 0.5f;
        return position + localCenter * scale;
    }
...
private:
    void ComputeLocalBounds()
    {
        ...
        glm::vec3 localCenter = (minBounds + maxBounds) * 0.5f;
        glm::vec3 extents = maxBounds - localCenter;
        boundingRadius = glm::length(extents);
    ...
    Mesh processMesh(aiMesh* mesh, const aiScene* scene)
    {
        ...
        // 1. GLTF base color (albedo)
        vector<Texture> baseColorMaps =
        ...
        // 4. GLTF metallic-roughness map
        vector<Texture> mrMaps =
